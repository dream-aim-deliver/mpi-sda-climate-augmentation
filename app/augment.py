...
def download_source_if_relevant(source: KernelPlancksterSourceData, job_id:int, tracer_id: str, log_level:str, scraped_data_repository: ScrapedDataRepository, work_dir: str):
    logger = logging.getLogger(__name__)
    logging.basicConfig(level=log_level)
    name = source["name"]
    ...
    if "climate" in source_data.relative_path and os.path.splitext(source_data.relative_path)[1] ==".json":
        ...
        try:
            ...
            logging.error(f"File not found in MinIO: {relative_path}")
            return res

        res["sentinel"] = True
    
    elif "Webcam" in source_data.relative_path and os.path.basename(source_data.relative_path) == "data.json":
